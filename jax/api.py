...
def deprecated_jit(fun: Callable) -> Callable:
  ...
  def f_jitted(*args, **kwargs):
    ...
    in_avals = tuple(raise_to_shaped(core.get_aval(x)) for x in args)
    jaxpr, consts, out_tree = trace_to_jaxpr(f, in_tree, in_avals)
    flat_fun = lu.hashable_partial(lu.wrap_init(core.eval_jaxpr), jaxpr, ())

    donated_invars = (False,) * (len(consts) + len(args))
    out = xla.xla_call(flat_fun, *consts, *args, device=None, backend=None,
                       name=fun.__name__, donated_invars=donated_invars)
    return tree_unflatten(out_tree, out)
